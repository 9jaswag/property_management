...
module ExceptionHandler
  ...
  def four_zero_zero(err)
    render json: { message: err.message }, status: :bad
  end

  # JSON response with message; Status code 498 - Expired token
  def four_nine_eight(err)
    render json: { message: err.message }, status: :invalid_token
  end

  # JSON response with message; Status code 409 - Conflict
  def four_zero_nine(err)
    render json: { message: err.message }, status: :conflict
  end
end
